...
module.exports = {
  ...
  deleteSeller: async (req, res) => {
    ...
    try {
      ...
      const produtos = await trx('PRODUTO').select('*').where('ID_USUARIO', id);
      produtos.forEach(async (produto) => {
        await trx('PRODUTOS_VENDIDOS').delete().where('ID_PRODUTO', produto.ID_PRODUTO);
      })
  ...
  updateSeller: async (req, res) => {
    ...
    const users = await db('USUARIO').select('EMAIL').where('ID_USUARIO', '!=', Id);
  
    if (!user) return res.json({ error: "Usuário não encontrado!" });

    var email = users.find(u => u.EMAIL === Email);
  ...
  getSpecificSeller: async (req, res) => {
    ...
    .groupBy(
      ...
      'USUARIO.DIAMES',
      'USUARIO.MES')
    .limit(5).where('USUARIO.ID_USUARIO', id);
