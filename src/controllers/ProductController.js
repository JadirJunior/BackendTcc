...
module.exports = {
  ...
  getProducts: async (req, res) => {
    ...
    try {
      ...
      if (idUsuario) {
          var products = await db('produto')
          .join('USUARIO', 'PRODUTO.ID_USUARIO', '=', 'USUARIO.ID_USUARIO')
          .select('USUARIO.USU_VEND_FIS', 
  ...
  addProduct: async (req, res) => {
    ...
    try {
      ...
      if (vendedor.length == 0)
        ...
        .andWhere('QUANTIDADE', Number(Quantidade))
        .andWhere('ID_USUARIO', idUsuario).first();
  ...
  getProductById: async (req, res) => {
    ...
    const product = await db('PRODUTO').select(
      'PRODUTO.*',  
      'IMAGENS_PRODUTOS.IMAGEM',
      'USUARIO.USU_VEND_FIS',
      'USUARIO.IMAGEM AS IMAGEM_USUARIO',
